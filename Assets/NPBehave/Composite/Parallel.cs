...
namespace NPBehave
{
    public class Parallel : Composite
    {
        ...
        protected override void DoStart()
        {
            ...
            {
                Assert.AreEqual(child.NodeState, eNodeState.eUNACTIVE);
            }
            ...
            succeededCount = 0;
            failedCount = 0;
            foreach (Node child in this.m_children)
            {
                runningCount++;
                child.Start();
            }
        ...
        protected override void DoChildStopped(Node child, bool result)
        {
            ...
            if (allChildrenStarted)
            {
                ...
                else if (!this.childrenAborted)
                {
                    ...
                    if (childrenAborted)
                    {
                        foreach (Node currentChild in this.m_children)
                        {
        ...
        public override void StopLowerPriorityChildren(Node abortForChild, bool immediateRestart)
        {
            if (immediateRestart)
            {
